...
const LiveCanvas: React.FC = () => {
  ...
  const connectToLive = async () => {
    ...
    try {
        ...
        if (!apiKey) {
            throw new Error("Gemini API key is not configured. Please ensure API_KEY is set in settings.");
        }
        ...
        // Connect Gemini Live with modern live model and fallback
        const connectSession = (modelName: string) => {
        ...
        try {
            sessionPromise = connectSession('gemini-2.5-flash-native-audio-preview-09-2025');
        } catch (err) {
            console.warn("Falling back to gemini-2.5-flash-live...", err);
            sessionPromise = connectSession('gemini-2.5-flash');
        }

        // Keep sessionRef to close later during cleanup
