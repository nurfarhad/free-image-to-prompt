...
const ImageUploader: React.FC<ImageUploaderProps> = ({ imageFile, onImageSelect, onEffectiveImageChange, heightClass = 'h-64' }) => {
  ...
  const fetchImageFromUrl = async () => {
      ...
      try {
          ...
          setUrlError("Could not load image. If the URL is valid, the host might block direct access (CORS). Try saving the image locally and uploading it.");
      } finally {
                    ...
                    {showUrlInput ? (
                        ...
                        >
                            ...
                            <input 
                                ...
                                className="w-full bg-zinc-950 border border-zinc-800 rounded-xl p-3 text-sm text-white mb-2 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none transition-all"
                                autoFocus
                            />
                            {urlError && (
                                <p className="text-[11px] text-red-400 bg-red-500/10 border border-red-500/20 rounded-lg p-2 mb-3 leading-relaxed">
                                    {urlError}
                                </p>
                            )}
