...
const formatGeminiError = (error: unknown): string => {
    if (error instanceof Error) {
        ...
        if (msg.includes("NOT_FOUND") || msg.includes("Requested entity was not found") || msg.includes("404")) {
            return "Model Not Available (404): The requested model was not found. It may require a paid API key or higher billing tier.";
        }
...
const getPromptSchema = (includeAB: boolean) => {
  const schema: any = {
    type: Type.OBJECT,
    properties: {
  ...
  if (includeAB) {
    schema.properties.abVariations = {
      type: Type.OBJECT,
      properties: {
        regular: { type: Type.STRING },
        creative: { type: Type.STRING }
      },
      required: ["regular", "creative"]
    };
    schema.required.push("abVariations");
  }
...
export const repairTruncatedJson = (json: string): string => {
  ...
  for (let i = 0; i < repaired.length; i++) {
    ...
    if (!inString) {
      if (char === '{') stack.push('}');
        ...
        else return repaired; // Malformed
      }
      else if (char === ']') {
        if (stack.length > 0 && stack[stack.length - 1] === ']') stack.pop();
        else return repaired; // Malformed
  ...
  while (stack.length > 0) {
    ...
    // Add comma if last char is a value and we are closing an object (very basic heuristic)
    if (repaired.endsWith('"') || /[\d\w]/.test(repaired[repaired.length-1])) {
        // We can't easily know if we need a closing quote or a comma, 
        // but adding closing brackets is the priority.
    }
    repaired += closing;
  }
...
export const generatePromptFromImage = async (
  ...
  onChunk?: (partial: Partial<PromptData>) => void
): Promise<PromptData> => {
  const { 
  ...
  **MODE GUIDELINES (${preset}):**
  ...
  if (coreTraits?.trim()) {
    systemInstruction += `\n\n**CHARACTER CONSISTENCY - GLOBAL CONSTRAINTS**: The following traits MUST be applied to the primary character in EVERY variation: "${coreTraits}". Ensure these traits (e.g. hair style, outfit constant, specific ethnicity) are prioritized as absolute consistency markers.`;
  }

  if (enableNegative) {
    systemInstruction += `\n\n**NEGATIVE PROMPT GENERATION**: Based on the image analysis, generate a "negative" field in the JSON string containing keywords of what to avoid to prevent common AI failures for this specific subject/style (e.g., "blurry", "extra limbs", "distorted face", "bad anatomy").`;
  }

  if (enableABMode) {
    systemInstruction += `\n\n**A/B COMPARISON MODE ENABLED**: You MUST populate the "abVariations" field in the root of the JSON object. 
    - "regular": A faithful, descriptive prompt based on the image and settings.
    - "creative": A highly artistic, stylized, or imaginative interpretation of the same subject, pushing the aesthetic boundaries while maintaining core subject features.`;
  }
  ...
  try {
    ...
    const response = await ai.models.generateContentStream({
      ...
      config: {
        ...
        responseSchema: getPromptSchema(!!enableABMode),
      },
    });
    ...
    try {
        ...
        console.warn("Stream interrupted or timed out:", streamError);
    }
    
    if (fullText) {
        try {
            const data = JSON.parse(fullText) as PromptData;
            data.isUniversal = isUniversal;
            data.isCharacterBuilder = isCharacterBuilder;
            return data;
        } catch (parseError) {
            // If final parse fails, try repaired version one last time
            const repaired = repairTruncatedJson(fullText);
            const data = JSON.parse(repaired) as PromptData;
            data.isUniversal = isUniversal;
            data.isCharacterBuilder = isCharacterBuilder;
            return data;
        }
  ...
  try {
    const response = await ai.models.generateContent({
      ...
      config: { 
          systemInstruction,
          responseMimeType: "application/json",
          responseSchema: getPromptSchema(false),
      },
...
export const generateStyleTransferPrompt = async (
    ...
    **OUTPUT GOAL**: Generate a prompt optimized for: ${options.preset || 'standard'}.`;
    ...
    if (options.coreTraits?.trim()) {
        systemInstruction += `\n**CHARACTER CONSISTENCY - GLOBAL CONSTRAINTS**: The following traits MUST be applied to the primary character: "${options.coreTraits}".`;
    }

    if (options.enableNegative) {
        systemInstruction += `\n**NEGATIVE PROMPT GENERATION ENABLED**: Include a "negative" field with avoid-keywords.`;
    }

    if (options.enableABMode) {
        systemInstruction += `\n**A/B COMPARISON MODE ENABLED**: Populate the "abVariations" field with "regular" and "creative" prompts.`;
    }
    ...
    try {
        const response = await ai.models.generateContentStream({
            ...
            config: { 
                systemInstruction,
                responseMimeType: "application/json",
                responseSchema: getPromptSchema(!!options.enableABMode)
            },
        ...
        try {
            ...
            console.warn("Style transfer stream interrupted:", streamError);
        }
  ...
  try {
    ...
    for (const model of modelsToTry) {
      try {
        ...
        if (errMsg.includes('404') || errMsg.includes('not_found') || errMsg.includes('not found') || errMsg.includes('unsupported')) {
          console.warn(`Model ${model} failed (${errMsg}), trying next available image model...`);
          continue;
        }
        throw err;
      }
...
export const editImage = async (base64Image: string, prompt: string): Promise<string> => {
    ...
    try {
        ...
        for (const model of modelsToTry) {
            try {
                response = await attemptEdit(model);
                if (response) break;
            } catch (err: any) {
                lastError = err;
                const errMsg = (err?.message || '').toLowerCase();
                if (errMsg.includes('404') || errMsg.includes('not_found') || errMsg.includes('not found') || errMsg.includes('unsupported')) {
                    console.warn(`Edit model ${model} failed (${errMsg}), trying next available model...`);
                    continue;
                }
                throw err;
            }
...
export const debloatPrompt = async (text: string): Promise<string> => {
    const ai = getAiClient();
    try {
        const response = await ai.models.generateContent({
            model: 'gemini-3-flash-preview',
            config: { systemInstruction: "Remove unnecessary filler words from this prompt while keeping all semantic details." },
            contents: { parts: [{ text: text }] }
        });
        return response.text || text;
    } catch (e) {
...
export const formatMidjourneyPrompt = async (text: string): Promise<string> => {
    const ai = getAiClient();
    try {
        const response = await ai.models.generateContent({
            model: 'gemini-3-flash-preview',
            config: { systemInstruction: "Format this image generation prompt specifically for Midjourney V6 style (shorter, descriptive phrases, separated by commas, avoiding complex sentences)." },
            contents: { parts: [{ text: text }] }
        });
        return response.text || text;
    } catch (e) {
